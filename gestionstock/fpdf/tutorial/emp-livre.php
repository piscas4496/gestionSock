...
{
	// Header
	$this->Image('emprunteurs.jpg',10,1,150);
	...
	$req=$db->query('SELECT * FROM emprunteurs ');

	// $this->Cell(190,12,'LES RESERVATIONS',1,0,'C');
		$this->Ln();
        $this->Cell(15,6,'ID',1,0,'C');
		$this->Cell(50,6,'NOM',1,0,'C');
		$this->Cell(62,6,'EMAIL',1,0,'C');
		// $this->Cell(25,6,'PRENOM',1,0,'C');
		$this->Cell(40,6,'DATE',1,0,'C');
		$this->Cell(18,6,'DELAI',1,0,'C');
		// $this->Cell(50,6,'Date de passation',1,0,'C');
		$this->Ln();

	while ($row = $req->fetch(PDO::FETCH_OBJ)) {
		
        $this->Cell(15,8,$row->id,1,0,'C');
		$this->Cell(50,8,$row->noms,1,0,'C');
		$this->Cell(62,8,$row->email,1,0,'C');
		// $this->Cell(25,8,$row->prenom,1,0,'C');
		$this->Cell(40,8,$row->date,1,0,'C');
		$this->Cell(18,8,$row->delai,1,0,'C');
